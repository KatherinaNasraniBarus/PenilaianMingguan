...
interface AppSidebarProps {
  ...
  /** localStorage key yang menyimpan nama user. Bisa string biasa atau JSON object dengan field `nama`. */
  userStorageKey: string;
  /** Label peran yang tampil di bawah nama, misal "Mahasiswa" atau "Mentor" */
  roleLabel: string;
}
...
export default function AppSidebar({
  ...
  useEffect(() => {
    ...
    if (!raw) return;
    // Coba parse JSON — kalau gagal berarti string biasa (nama langsung)
    try {
      const parsed = JSON.parse(raw);
      setDisplayName(parsed.nama ?? raw);
    } catch {
      setDisplayName(raw);
    }
  }, [userStorageKey]);
  ...
  return (
    <>
      ...
      <aside
        className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-white border-r border-emerald-100
          flex flex-col h-full shrink-0 transform transition-transform duration-300 ease-in-out
      ...
      >
              ...
              >
                <Icon
                  ...
                  className={
                    isActive
                      ? "text-white"
                      : "text-emerald-500 group-hover:text-emerald-700 transition-colors"
                  }
                />
        ...
        <div className="p-4 border-t border-emerald-50 bg-slate-50/50 shrink-0">
          ...
          <button
            ...
            className="w-full flex items-center justify-start gap-3 px-4 py-2.5 rounded-xl
              text-red-600 hover:bg-red-50 hover:text-red-700 font-bold transition-all
              border border-transparent hover:border-red-100 group active:scale-95"
          >
            <LogOut
              size={18}
              strokeWidth={2.5}
